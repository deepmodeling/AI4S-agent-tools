...
# 1. 核心构建环境准备
# -----------------------------------------------------------------------------
# 修正点：添加 scikit-build-core (DeepMD-kit 新版构建后端依赖)
# 同时保留 scikit-build, cmake, ninja, packaging, distro, pathspec, pyproject_metadata
# pathspec 和 pyproject_metadata 通常是 scikit-build-core 的依赖，显式安装更稳妥
...
RUN python -c "import torch; print('Successfully imported torch')"FROM registry.dp.tech/dptech/deepmd-kit:3.1.1
